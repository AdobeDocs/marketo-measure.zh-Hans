...
1. 要开始设置Marketo活动，请导航到&#x200B;**我的帐户** > **设置** > **活动**。
   ...
   ![1. 要开始设置Marketo活动，请导航到“我的帐户设置”](assets/marketo-engage-activities-01.png)

   ![1. 要开始设置Marketo活动，请导航到“我的帐户设置”](assets/marketo-engage-activities-02.png)
...
1. 单击[!UICONTROL Select Activities Types]下的下拉菜单开始选择各种类型。
   ...
   ![1. 单击“Select Activities Types to begin](assets/marketo-engage-activities-03.png)（选择活动类型）”下的下拉菜单

1. 选择所需的所有活动后，您会看到它们已填充到[!UICONTROL Selected Activities List]和[!UICONTROL Define Rules]下。

   ![1. 选择所有需要的活动后，您会看到](assets/marketo-engage-activities-04.png)
...
1. 首先，根据活动类型，您可能需要设置稍后可用于渠道映射的[!DNL Marketo Measure]促销活动名称。 [!DNL Marketo Measure]营销活动名称可在多个规则中重用。 这有助于在单个渠道规则中使用更广泛的名称。 并非所有活动类型都包含Marketo程序，因此第一步，需要指定一个名称。
   ...
   ![以下是该额外步骤的示例：](assets/marketo-engage-activities-05.png)
...
1. 在“更改得分”示例中，需要输入“促销活动名称”，因为该信息是从Marketo项目中提取的。 现在创建规则表达式。 在此示例后，选择值为90且运算符为“[!UICONTROL New Value]”的字段“[!UICONTROL is greater than]”。
   ...
   ![您可以展开规则并添加其他筛选器或条件](assets/marketo-engage-activities-06.png)

   ![您可以展开规则并添加其他筛选器或条件](assets/marketo-engage-activities-07.png)

1. 最后，选择要用作接触点日期的内容。 所有可用的日期或日期/时间字段均在此处显示Marketo。 除非您有自定义日期字段，否则您会看到&quot;[!UICONTROL Activity Date]&quot;。

   ![1. 最后，选择要用作接触点日期的内容。 所有可用](assets/marketo-engage-activities-08.png)

1. 请确保在此过程中单击&#x200B;**[!UICONTROL Save As Draft]**，以便您不会丢失所做的更改。

   ![1. 请确保沿途单击“另存为草稿”](assets/marketo-engage-activities-09.png)

1. 导航到&#x200B;**[!UICONTROL Attribute Mapping]**&#x200B;选项卡。

   ![1. 导航到“属性映射”选项卡。](assets/marketo-engage-activities-10.png)
...
对于具有项目ID的活动类型中的任意规则，Marketo项目渠道由项目确定。 我们使用项目频道来映射到您的自定义脱机频道，因此您需要确保按照此处[的说明正确配置了您的频道](/help/marketo-engage-programs-integration.md)。

对于没有项目ID的活动类型中的任何规则，您的第一步是创建营销活动名称。 使用此促销活动名称可设置您在此处[布局的自定义在线渠道](/help/channel-tracking-and-setup/online-custom-channel-setup.md)。
...
Mark点击了一个Facebook广告并登陆到wayneenterprises.com ，在那里他获得一个Cookie，ID为[!DNL Marketo Measure] 123和[!DNL Marketo Munchkin Id] 456。 没有表单填写。

Wayne Enterprises营销团队向特定的目标潜在客户发送电子邮件爆炸邮件，其中一个是`mark@email.com`。

`mark@email.com`收到电子邮件后点进并登陆`wayneenterprises.com`。 对`mark@email.com's`的第二次访问具有相同的Cookie Id，但是没有表单填写，因此对于`wayneenterprise.com`，他们仍然是匿名访客。[!DNL Marketo Measure]
