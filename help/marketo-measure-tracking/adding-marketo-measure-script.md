...
可在[此处](/help/marketo-measure-tracking/adding-marketo-measure-script-via-google-tag-manager.md){target="_blank"}找到更多信息。
...
您是否有任何表单不应[!DNL Marketo Measure]跟踪，因为它们对归因不一定有意义（例如，取消订阅表单、客户登录等）？ 如果是，则要将此文章[中的排除代码](/help/marketo-measure-tracking/excluding-marketo-measure-from-specific-forms.md){target="_blank"}添加到每个表单
...
* 解决方案：您需要强制其中一个表单根据缓存的数据向[!DNL Marketo Measure]报告用户并讨论放弃实践。 通常，[报告用户代码](/help/marketo-measure-tracking/ajax-form-handling.md){target="_blank"}可以解决此问题。
...
* 解决方案：您的Web开发团队可以使用这些[报告用户模板](/help/marketo-measure-tracking/ajax-form-handling.md){target="_blank"}来添加所需的代码。
