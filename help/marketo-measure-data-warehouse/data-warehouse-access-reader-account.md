---
description: Data Warehouse访问权限 — Reader帐户
...
1. 在[!DNL Marketo Measure]中，单击页面顶部的&#x200B;**[!UICONTROL My Account]** > **[!UICONTROL Settings]**。
   ...
   ![包含“我的帐户”和“设置”选项的Marketo Measure导航菜单](assets/data-warehouse-access-reader-account-1.png)

1. 在左侧菜单的“安全”下，单击&#x200B;**[!UICONTROL Data Warehouse]**。

   ![设置侧边栏，带有“安全”部分下的Data Warehouse选项](assets/data-warehouse-access-reader-account-2.png)

1. 此页面包含指向Snowflake Data Warehouse和用户名的链接。

   显示Data Warehouse URL和用户名的![Snowflake信息页](assets/data-warehouse-access-reader-account-3.png)
...
1. 单击Snowflake URL中提供的链接，这会将您转到Snowflake登录页面，您可以在其中输入用户名和密码。 _如果没有密码，请参阅以下步骤重置密码_。
   ...
   ![包含用户名和密码字段的Snowflake登录页面](assets/data-warehouse-access-reader-account-4.png)

1. 登录后，单击页面顶部的&#x200B;**[!UICONTROL Worksheets]**。

   带有工作表导航选项的![Snowflake界面](assets/data-warehouse-access-reader-account-5.png)

1. BIZIBLE_ROI_V3数据库对象位于屏幕左侧。 在查询窗口顶部的下拉选项中输入仓库、数据库和方案。 每个选项应只有一个选项。 现在，您可以在Snowflake查询编辑器中执行查询了。

   具有BIZIBLE_ROI_V3数据库和下拉选择器的![Snowflake查询编辑器](assets/data-warehouse-access-reader-account-6.png)
...
使用“重置密码”按钮的![Data Warehouse页面](assets/data-warehouse-access-reader-account-7.png)

![密码重置后临时密码显示](assets/data-warehouse-access-reader-account-8.png)
...
* **仓库名称** （并非总是必需的）
  ...
  ![显示数据库和仓库名称详细信息的Data Warehouse信息页](assets/data-warehouse-access-reader-account-9.png)
