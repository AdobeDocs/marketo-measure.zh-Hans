...
有关每个Marketo Measure字段都相关的对象的映射，请 [单击此处](/help/configuration-and-setup/marketo-measure-and-salesforce/marketo-measure-object-and-field-taxonomy.md).
...
**广告营销活动ID** |在买方接触点、买方归因接触点上找到

填充此字段的方式有三种：

`1)` 如果接触点来自付费搜索工作（AdWords或BingAds），则广告平台中的广告促销活动ID将显示在此处。
...
在此示例中，广告促销活动ID将显示： __GAId__ 营销虚拟活动2014年9月

`3)` 如果接触点来自离线Salesforce营销活动（会议、晚餐等），则广告营销活动ID将呈现Salesforce营销活动ID

如果以上都不包含，此字段将为空。

**广告营销活动名称** |买方接触点、买方归因接触点
...
`4)` 这将使用为在Marketo Measure帐户中构建的活动生成的接触点定义的Campaign名称填充。
...
`1)` 如果接触点来自付费搜索(AdWords/Bing Ads)，则字段将显示广告平台的完整广告副本。
...
**广告目标URL** |买方接触点、买方归因接触点

`1)` 如果接触点来自付费搜索，则此字段将显示单击搜索引擎中的广告后您被定向到的URL目标。
...
`1)` 如果接触点来自付费搜索，则此处将显示来自AdWords/Bing Ads的广告组名称。
...
如果您使用的是自定义归因模型，此字段将以小数形式显示根据自定义模型中设置的值分配给接触点的归因点数的百分比。 由于此字段与买方接触点对象相关，因此它并非收入点数的反映，而仅仅是归因点数的反映。
...
由于此字段与买方接触点对象相关，因此它并非收入点数的反映，而仅仅是归因点数的反映。
...
**关键字MatchType** |买方接触点、买方归因接触点

如果接触点来自付费搜索，则此字段将显示广告平台(Adwords/Bing)中的匹配类型。

**关键字文本** |买方接触点、买方归因接触点

`1)` 如果接触点来自付费搜索，则此字段将显示广告平台中的关键词文本(Adwords/BingAds)或登陆页面URL中_bk参数的值。
...
`2)` 如果接触点并非来自付费搜索，则此字段将显示登陆页面URL中的utm_term值。
...
**登陆页面 — 原始** |买方接触点、买方归因接触点

此字段显示在Web会话期间访问的第一个网页的整个URL（包括UTM参数）。

**商机** |买方接触点、Marketo Measure人员
...
此字段显示接触点所属的营销活动或营销渠道的一般组（即付费搜索、直接、社交等）。 根据Marketo Measure应用程序中渠道的设置方式，对接触点进行分组。 有关营销渠道或如何设置渠道的更多信息，请 [单击此处](/help/channel-tracking-and-setup/online-channels/online-custom-channel-setup.md).

**营销渠道 — 路径** |买方接触点、买方归因接触点
...
`1)` 如果接触点来自付费搜索，则来自Adwords/BingAds的媒体将显示在此处（即CPC）。

`2)` 如果接触点不是来自付费搜索，则此字段显示登陆页面URL中的utm_medium值。

`3)` 如果接触点来自离线营销活动，则此字段将在Salesforce营销活动中显示“类型”字段。

`4)` 这将使用生成接触点的相关活动中的“活动类型”值填充。

如果以上均不提供，Marketo Measure会自动设置“中”值。
...
**Salesforce Campaign** |买方接触点、买方归因接触点
...
`1)` 如果接触点来自在线来源，则此字段将显示接触点发生的日期和时间。
...
`4)` 如果接触点从CRM Campaign生成，则将填充为CRM Campaign。

`5)` 如果接触点从CRM活动生成，则此项将填充为CRM活动。

如果以上都不包含，则此字段将填充为“Web直接”或“Web”。

**接触点源(FT)** |买方接触点

此字段与接触点来源相同，但是此字段专门显示首次接触点的来源。

**接触点源(LC)** |买方接触点

这是与“接触点来源”相同的字段，但此字段专门显示“潜在客户创建”接触点的来源。
