...
1. 在左侧的设置选项列中，单击&#x200B;**[!UICONTROL Connections]**&#x200B;部分下的[!UICONTROL Integrations]。
   ...
   ![设置菜单，显示“集成”部分下的“连接”选项](assets/connect-marketo-measure-to-salesforce-1.png)

1. 在“连接”中的CRM部分下，单击&#x200B;**[!UICONTROL Set Up New CRM Connection]**。

   ![设置新CRM连接的CRM连接页面](assets/connect-marketo-measure-to-salesforce-2.png)

1. 此时会出现一个弹出窗口，要求您选择CRM连接。 单击&#x200B;**[!UICONTROL Connect]**&#x200B;徽标旁边的[!DNL Salesforce]。

   ![选择带有Salesforce徽标和连接按钮的CRM连接弹出窗口](assets/connect-marketo-measure-to-salesforce-3.png)
...
![每日CRM API限制设置显示API信用配额的配置字段](assets/connect-marketo-measure-to-salesforce-4.png)
...
1. 单击“锁定”图标以进行编辑。
   ...
   ![用于编辑每日CRM API限制字段的锁定图标](assets/connect-marketo-measure-to-salesforce-5.png)

1. 输入等于或大于100,000的所需限制。 完成后单击&#x200B;**保存**。

   ![已输入值的每日CRM API限制输入字段并保存按钮](assets/connect-marketo-measure-to-salesforce-6.png)
