...
启用此功能后，导出作业将停止前进并保留在出现错误的记录上，直到问题得到解决。 这些错误通常是由缺少权限、未正确应用自定义验证规则或工作流/触发器中的问题造成的。 作业将继续按计划运行，并将自动重新尝试导出失败的记录，直到成功为止。

如果选择禁用此功能，会出现一个警告弹出窗口，通知您这可能导致数据不一致。 您有责任解决因这些不一致而可能出现的任何问题。

无论该功能是否切换，遇到的所有记录级错误都记录在`ExportErrors`表中，`CRMExport_ExportError`作业每天将自动尝试重新导出这些记录。 这消除了支持请求启动重新导出的需要，因为重新导出是自动进行的，无需任何开发人员干预。
