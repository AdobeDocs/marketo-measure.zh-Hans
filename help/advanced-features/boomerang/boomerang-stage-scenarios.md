...
![显示MQL和SQL boomerang接触点的Lead历程图，按时间顺序列为Opportunity](assets/1-1.png)

**方案2：潜在客户的回访式接触点和自定义阶段**

在此方案中，客户选择只跟踪带有回滚接触点的&#x200B;**SQL阶段**。 仍在跟踪MQL和SAL阶段，但具有[!DNL Marketo Measure]自定义阶段功能。

![显示将MQL和SAL作为自定义阶段进行跟踪的SQL boomerang接触点的潜在客户历程](assets/2-1.png)
...
![潜在客户历程显示跳过的SAL阶段，SAL和OC位置组合在同一接触点中](assets/3.png)
...
![显示MQL和SAL boomerang接触点转换为Opportunity的三个潜在客户历程图](assets/4.png)
...
![在Opportunity上显示位置编号的MQL、SQL和SAL boomerang接触点的三个潜在客户历程](assets/5.png)
...
![三个潜在客户历程，显示在SQL阶段之后发生Web访问时从Opportunity中排除的Boomerang接触点](assets/6.png)
...
![显示具有多阶段过渡的boomerang归因权重的三个潜在客户历程](assets/7.png)
...
![Contact journey显示Opportunity上的OC接触点之后出现的演示和协商Boomerang接触点](assets/8.png)
