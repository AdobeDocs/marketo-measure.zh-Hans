...
如果您是新[!DNL Marketo Measure]客户，则可能有兴趣提供一些通过SFDC Campaigns跟踪的营销数据。 但是，如果您要为您的在线SFDC促销活动启用接触点，则可能会遇到重复计算归因的问题，因为[!DNL Marketo Measure]会自动为您的在线营销工作创建接触点。 为避免重复计算数据，您可以使用Campaign接触点结束日期对[!DNL Marketo Measure]在SFDC营销活动中创建的接触点日期设置限制。 例如，如果您希望为一直在SFDC中跟踪的社交活动添加追溯转化，但您知道已在7月1日添加[!DNL Marketo Measure]个JavaScript（该活动正在创建在线接触点），则可以编辑Social SFDC营销活动以包含等于7月1日的接触点结束日期，并为该营销活动启用购买者接触点。
