...
[!DNL Marketo Measure]处理系统将不会注册已从SFDC中手动删除接触点。 截至目前，还没有触发程序向我们的系统发出信号以调整数据。[!DNL Marketo Measure] 将不会自动推送另一个接触点来替换已删除的接触点，也不会将该接触点位置或归因重新分配给后续接触点。
...
![删除接触点时，会在归因中创建一个洞](assets/touchpoint-settings-10.png)
