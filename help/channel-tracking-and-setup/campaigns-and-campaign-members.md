---
description: 面向Marketo Measure用户的营销活动和营销活动成员指南
title: 营销活动和营销活动成员
...
>[!NOTE]
>
>本文介绍了一个过时的流程。 我们鼓励用户使用[新的、改进的应用程序内进程](/help/channel-tracking-and-setup/custom-campaign-sync.md){target="_blank"}。

## 启用接触点 {#enabling-touchpoints}

[!DNL Marketo Measure] [!DNL Salesforce]包将在营销活动对象中包含一个标记为“启用购买者接触点”的字段。 将字段添加到页面布局后，它将显示为以下形式：

![Marketo Measure Salesforce包将包含一个标记为](assets/dynamics-lists-1.png)的字段

选择列表中可用的选项包括：

![选择列表中可用的选项为：](assets/dynamics-lists-10.png)
...
![如果在使用Marketo Measure之前，您使用的是营销自动化平台](assets/dynamics-lists-2.png)
...
![如果启用，Marketo Measure会将状态值推送到](assets/dynamics-lists-3.png)
...
![如果列表是在](assets/dynamics-lists-4.png)之后的天/周/月上传的，则可能必须执行此操作
...
![已安装的Marketo中包含批量更新接触点日期](assets/dynamics-lists-5.png)
...
![有一个搜索不起作用，该搜索显示在](assets/legacy-processes-10.png)中
...
![设置日期和时间后，单击更新选定项](assets/dynamics-lists-6.png)

## 营销活动成本 {#campaign-costs}

请参阅本文[以了解有关Campaign成本](/help/crm-campaign-costs.md){target="_blank"}的所有信息。
