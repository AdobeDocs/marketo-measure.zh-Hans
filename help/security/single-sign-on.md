...
>[!NOTE]
>公司可能使用不同的身份提供程序（例如，Ping身份、Okta）。 以下设置说明和UI中使用的术语可能与您的身份提供程序使用的术语不直接匹配。
...
![具有自定义SSO登录类型选项的身份验证设置页面](assets/single-sign-on-1.png)
...
具有用户属性映射字段的![SAML配置表单](assets/single-sign-on-2.png)
...
![具有标准用户和管理员选项的用户角色映射配置](assets/single-sign-on-3.png)
...
![成功的SSO登录确认屏幕](assets/single-sign-on-6.png)
